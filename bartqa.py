...
vehicles = getActors()
qas = pd.DataFrame(columns=['Scene ID','Scene','Question','Answer','Answer LOSS'])
# print(vehicles)
sentVehicle = {}
#Analyze actions
prevSent = ''
...
q,ans, los = getQA('what is the type of the road?', 'one-way,two-way,junction')
qas.loc[qas.shape[0]] = [0, text2, q, ans[0], los]
q,ans, los = getQA('is the road a junction?', 'yes,no')
qas.loc[qas.shape[0]] = [0, text2, q, ans[0], los]
q,ans, los = getQA('what is the type of the junction?', 't-junction,4-way junction')
qas.loc[qas.shape[0]] = [0, text2, q, ans[0], los]
q,ans, los = getQA('is the junction a t-junction?', 'yes,no')
qas.loc[qas.shape[0]] = [0, text2, q, ans[0], los]
q,ans, los = getQA('is the junction 4-way?', 'yes,no')
qas.loc[qas.shape[0]] = [0, text2, q, ans[0], los]
q,ans, los = getQA('How many branches are there?', '1,2,3,4')
qas.loc[qas.shape[0]] = [0, text2, q, ans[0], los]
...
qas.loc[qas.shape[0]] = [0, text2, q, ans[0], los]
q,ans, los = getQA('How many lanes are on the road?', '1,2,3')
qas.loc[qas.shape[0]] = [0, text2, q, ans[0], los]
...
for i in range(len(vehicles)):
    vehicle = vehicles[i]
    q,ans,los = getQA('what is ' + vehicle['phrase'] + ' doing?', 'travel,turn,stop,park')
    qas.loc[qas.shape[0]] = [0,text2,q,ans[0],los]
    q,ans, los = getQA('is ' + vehicle['phrase'] + ' turning?', 'yes,no')
    qas.loc[qas.shape[0]] = [0, text2, q, ans[0], los]
    q,ans, los = getQA('where is ' + vehicle['phrase'] + ' turning?', 'left,right')
    qas.loc[qas.shape[0]] = [0, text2, q, ans[0], los]
    q,ans, los = getQA('is ' + vehicle['phrase'] + ' turning left?', 'yes,no')
    qas.loc[qas.shape[0]] = [0, text2, q, ans[0], los]
    q,ans, los = getQA('is ' + vehicle['phrase'] + ' turning right?', 'yes,no')
    qas.loc[qas.shape[0]] = [0, text2, q, ans[0], los]
    q,ans, los = getQA('which direction is ' + vehicle['phrase'] + ' turning onto?', 'south,north,east,west')
    qas.loc[qas.shape[0]] = [0, text2, q, ans[0], los]
    q,ans, los = getQA('is ' + vehicle['phrase'] + ' turning onto south?', 'yes,no')
    qas.loc[qas.shape[0]] = [0, text2, q, ans[0], los]
    q,ans, los = getQA('is ' + vehicle['phrase'] + ' turning onto north?', 'yes,no')
    qas.loc[qas.shape[0]] = [0, text2, q, ans[0], los]
    q,ans, los = getQA('is ' + vehicle['phrase'] + ' turning onto east?', 'yes,no')
    qas.loc[qas.shape[0]] = [0, text2, q, ans[0], los]
    q,ans, los = getQA('is ' + vehicle['phrase'] + ' turning onto west?', 'yes,no')
    qas.loc[qas.shape[0]] = [0, text2, q, ans[0], los]
    q,ans, los = getQA('is ' + vehicle['phrase'] + ' travelling?', 'yes,no')
    qas.loc[qas.shape[0]] = [0, text2, q, ans[0], los]
    q,ans, los = getQA('is ' + vehicle['phrase'] + ' stopped?', 'yes,no')
    qas.loc[qas.shape[0]] = [0, text2, q, ans[0], los]
    q,ans, los = getQA('direction of ' + vehicle['phrase'] + ' ?', 'south,north,east,west')
    qas.loc[qas.shape[0]] = [0, text2, q, ans[0], los]
    ...
    qas.loc[qas.shape[0]] = [0, text2, q, ans[0], los]
    q,ans, los = getQA('where is ' + vehicle['phrase'] + ' parked?', 'on the road,off-street')
    qas.loc[qas.shape[0]] = [0, text2, q, ans[0], los]
    q,ans, los = getQA('is ' + vehicle['phrase'] + ' parked on the road?', 'yes,no')
    qas.loc[qas.shape[0]] = [0, text2, q, ans[0], los]
    q,ans, los = getQA('is ' + vehicle['phrase'] + ' parked off-street?', 'yes,no')
    qas.loc[qas.shape[0]] = [0, text2, q, ans[0], los]
    q,ans, los = getQA('which side is ' + vehicle['phrase'] + ' parked on?', 'left,right')
    qas.loc[qas.shape[0]] = [0, text2, q, ans[0], los]
    ...
    #Relative location comparison
    for j in range(i):
        v2 = vehicles[j]
        q,ans, los = getQA('which vehicle is in front?', vehicle['phrase']+','+v2['phrase'])
        qas.loc[qas.shape[0]] = [0, text2, q, ans[0], los]
        q,ans, los = getQA('is ' + vehicle['phrase'] + ' in front of ' + v2['phrase'] + ' ?', 'yes,no')
        qas.loc[qas.shape[0]] = [0, text2, q, ans[0], los]
        q,ans, los = getQA('is ' + v2['phrase'] + ' in front of ' + vehicle['phrase'] + ' ?', 'yes,no')
        qas.loc[qas.shape[0]] = [0, text2, q, ans[0], los]


qas.to_excel('QAquality.xlsx')
